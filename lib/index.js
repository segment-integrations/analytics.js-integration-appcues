 ...
 * Screen.
 *
 * http://appcues.com/docs#screen
 *
 * @api public
 * @param {Screen} screen
 */

Appcues.prototype.screen = function(screen) {
  if (!this.loaded()) {
    console.log('Appcues is not loaded');
    return;
  }

  if (window.Appcues.screen) {
    window.Appcues.screen(screen.name(), screen.properties());
  } else {
    // Legacy SDK
    window.Appcues.start();
  }
